...
p1 <- ggplot(density_data, aes(x = y)) +
  ...
  geom_line(aes(y = normal_density, color = "Standard Normal"), linetype = "dashed", size = 1.2) +  # Standard normal density
  labs(title = expression(paste("Target Density vs ", N(mu, sigma^2))),
       x = "y",
       y = "Density") +
  scale_color_manual(values = c("Target Density" = "coral3", "Standard Normal" = base)) +  # Color settings
  theme_bw() +
...
###Structuring
 ...
 - Implement S3-sampler for each sampler.

 - Chooses the Gaussian envelope numerically every time - employing the same method we did.
 
 - The breakpoints for the log-affine envelope are chosen as an equidistant grid between 0 and 0.35, the number is set to 10 by default.
...
bm_l <- bench::press(
  ...
  {
    bench::mark(
      Gaussian = gaus_rej_f(n, mu_opt, sigma_opt, alpha_s, tar_dens),
      log_affine = aff_rej_n(n, n_breakpoints),
      iterations = 10,
      check = FALSE
    )
  }
...
---
###Optimization
Based on the profiling, we can see that we spend some time in the loop. We have investigated two approaches:

- Parallelization
Seeing as we are performing independent operations, one approach is to parallelize the code. We will use `foreach` and `doParallel` to do this.

- Implementing in `Rcpp`
Another approach is to implement the code in `Rcpp`.

```{r,echo=F, warning=FALSE, message=FALSE}
source("C:/Users/birgi/Documents/comp_stat/Dina/Assignment 2 - Rejection sampling/Soure_code.R")

# Define the number of samples
n_samples <- 10^seq(1,3)
...
bm_l <- bench::press(
  ...
  {
    bench::mark(
      parallelized = aff_rej_par(n, n_breakpoints),
      log_affine = aff_rej_s(n, n_breakpoints),
      vectorized = aff_rej_n(n, n_breakpoints),
      iterations = 10,
      check = FALSE
    )
  }
...
ggplot(plot_data, aes(x = n, y = median, color = expr)) +
  ...
  scale_y_log10() +  # Log scale for better readability
  scale_color_manual(
    values = c("parallelized" = "#4f7942", "log_affine" = "coral3")
  ) +
  labs(x = "number of samples", y = "Time (ms) - logscale", color = "Method") +
  theme_bw() +
  ggtitle("Development for no. of samples") + my_theme
