...
$$
And implement two different methods for bandwidth selection:
 - AMISE
 - Cross-validation
As the selection of the bandwidth is highly important for the performance of the kernel density estimator.
---
...
$$AMISE(h)=\frac{h^4}{36}||f_0''||_2^2+\frac{1}{2Nh}$$
Differentiating and setting equal to 0 yields the optimal bandwidth. (Works well as AMISE goes to $\infty$ for $h\rightarrow 0$ and $h\rightarrow \infty$):
$$h_{N}=\left(\frac{9}{2||f_0''||_2^2}\right)^{1/5}N^{-1/5}$$
...
We test our kernel estimators on the simulated data from $\mathcal{N}(0.25,2.4)$ and compare them with the `density()` function in base `R`.
...
h_values <- data.frame(
  method = c( "AMISE", "CV"),
  h = c(norm_am$density$h, norm_cv$density$h)
)

knitr::kable(h_values)
...
```{r, echo = FALSE, message = FALSE, warning=FALSE, fig.align='center', fig.width=11, fig.height=7}
