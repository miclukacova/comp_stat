...
cppFunction('NumericVector gradient_rcpp(NumericVector par, int i, NumericVector x, NumericVector y) {
  i = i - 1;
  ...
  // Calculate the exp(beta * log(x_i) - alpha) term
  double expbetalogxalpha = exp(beta * log(x_i) - alpha);
  double identical_part = -2 * (y_i - f_x_i);

  // Compute the gradients
  double grad_alpha = identical_part * (rho - gamma) * expbetalogxalpha / pow(1 + expbetalogxalpha, 2);
  double grad_beta = -identical_part * (rho - gamma) * log(x_i) * expbetalogxalpha / pow(1 + expbetalogxalpha, 2);
  double grad_gamma = identical_part * (1 - 1 / (1 + expbetalogxalpha));
  double grad_rho = identical_part / (1 + expbetalogxalpha);
