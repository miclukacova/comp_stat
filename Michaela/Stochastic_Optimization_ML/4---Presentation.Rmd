...
We do a naive test of the algorithm, and run the algorithm with 3 different starting values: the true values $(2,5,1,2)$, $(1,1,1,1)$ and random values $(1.88, 0.31, 3.64, 2.29)$. Step size is $0.005$, maximal number of iterations $100$.  
...
sgd1 <- SGD(par0 = sv1, grad = grad, gamma = 0.005, x = x, y = y, true_par = param$par)
sgd2 <- SGD(par0 = sv2, grad = grad, gamma = 0.005, x = x, y = y, true_par = param$par)
sgd3 <- SGD(par0 = sv3, grad = grad, gamma = 0.005, x = x, y = y, true_par = param$par)
```
...
- $\gamma_0 = 1$, $n_1 = 100$, $\gamma_1 = 0.01$ and $a = 1$. 
- $\gamma_0 = 1$, $n_1 = 100$, $\gamma_1 = 0.01$ and $a = 0.6$. 

```{r, warning = FALSE, message = FALSE, echo = FALSE}
decay1 <- decay_scheduler(gamma0 = 1, a = 1, K = 0.4)
decay2 <- decay_scheduler(gamma0 = 1, a = 1, n1 = 100, gamma1 = 0.01)
decay3 <- decay_scheduler(gamma0 = 1, a = 0.6, n1 = 100, gamma1 = 0.01)
```
...
ggplot() +
  ...
  geom_line(aes(x = plot_data(sgd_decay2)$.time, 
                y = plot_data(sgd_decay2)$abs_dist_from_par, color = "a = 1, gamma1 = 0.01")) +
  geom_line(aes(x = plot_data(sgd_decay3)$.time, 
                y = plot_data(sgd_decay3)$abs_dist_from_par, color = "a = 0.6, gamma1 = 0.01")) +
  scale_y_log10() +
...
We compare the performance of the two algorithms SGD and GD with momentum. We use the same data as before and start the algorithms off in the points $(2.2, 5.2, 1.2, 2.2)$. 

```{r, echo = FALSE}
gd0 <- GD(par = param$par + rep(0.2,4), H = H, x = x, y = y, true_par = param$par, alg = grad_desc_mom)
sgd0 <- SGD(par0 = param$par + rep(0.2,4), grad = grad_rcpp, x = x, y = y, true_par = param$par, 
            gamma = 0.01)
```
...
# Store all plots in a list
adam_SGD_plots <- lapply(adam_SGD_objects, create_SGD_plot, plot_no = 1)
...
decaySGD <- decay_scheduler(gamma0 = 1, a = 0.5, n1 = 150, gamma1 = 0.01)
...
my_color_scale <- scale_color_manual(
  ...
  values = c("GD" = "hotpink4", "SGD" = "tan4", "MB" = "steelblue4", 
             "Momentum" = "#4f7942", "Adam" = "coral4")
)
...
l_time_1 <- ggplot(d_gd1, aes(x = .time, y = loss, color = "GD")) +
      geom_line() +
      scale_y_log10() +
      labs(title = "N = 100", x = "Time", y = "Loss") + 
  geom_line(aes (y = d_sgd1$loss, color = "SGD")) +
  geom_line(aes (y = d_mb1$loss, color = "MB")) +
  geom_line(aes (y = d_mom1$loss, color = "Momentum")) +
  geom_line(aes (y = d_ad1$loss, color = "Adam")) +
  my_color_scale +
  my_theme
               
l_time_2 <- ggplot() +
      geom_line(aes(x = d_gd2$.time, y = d_gd2$loss, color = "GD")) +
      scale_y_log10() +
      labs(title = "N = 1000", x = "Time", y = "Loss") + 
  geom_line(aes (x = d_sgd2$.time, y = d_sgd2$loss, color = "SGD")) +
  geom_line(aes (x = d_mb2$.time, y = d_mb2$loss, color = "MB")) +
  geom_line(aes (x = d_mom2$.time, y = d_mom2$loss, color = "Momentum")) +
  geom_line(aes (x = d_ad2$.time, y = d_ad2$loss, color = "Adam")) +
  my_color_scale+
  my_theme
...
l_time_3 <- ggplot() +
  ...
  geom_line(aes (x = d_sgd3$.time, y = d_sgd3$loss, color = "SGD")) +
  geom_line(aes (x = d_mb3$.time, y = d_mb3$loss, color = "MB")) +
  geom_line(aes (x = d_mom3$.time, y = d_mom3$loss, color = "Momentum")) +
  geom_line(aes (x = d_ad3$.time,y = d_ad3$loss, color = "Adam")) +
  my_color_scale+
  my_theme
...
```{r, echo = FALSE, fig.retina = 2, fig.width = 16, fig.height = 7, class = "center", warning = FALSE}
a_time_1 <- ggplot(d_gd1, aes(x = .time, y = abs_dist_from_par, color = "GD")) +
      geom_line() +
      scale_y_log10() +
      labs(title = "N = 100", x = "Time", y = "abs_dist_from_par") + 
  geom_line(aes (y = d_sgd1$abs_dist_from_par, color = "SGD")) +
  geom_line(aes (y = d_mb1$abs_dist_from_par, color = "MB")) +
  geom_line(aes (y = d_mom1$abs_dist_from_par, color = "Momentum")) +
  geom_line(aes (y = d_ad1$abs_dist_from_par, color = "Adam")) +
  my_color_scale +
  my_theme
               
a_time_2 <- ggplot() +
      geom_line(aes(x = d_gd2$.time, y = d_gd2$abs_dist_from_par, color = "GD")) +
      scale_y_log10() +
      labs(title = "N = 1000", x = "Time", y = "abs_dist_from_par") + 
  geom_line(aes (x = d_sgd2$.time, y = d_sgd2$abs_dist_from_par, color = "SGD")) +
  geom_line(aes (x = d_mb2$.time, y = d_mb2$abs_dist_from_par, color = "MB")) +
  geom_line(aes (x = d_mom2$.time, y = d_mom2$abs_dist_from_par, color = "Momentum")) +
  geom_line(aes (x = d_ad2$.time, y = d_ad2$abs_dist_from_par, color = "Adam")) +
  my_color_scale+
  my_theme

a_time_3 <- ggplot() +
      geom_line(aes(x = d_gd3$.time, y = d_gd3$abs_dist_from_par, color = "GD")) +
      scale_y_log10() +
      labs(title = "N = 10.000", x = "Time", y = "abs_dist_from_par") + 
  geom_line(aes (x = d_sgd3$.time, y = d_sgd3$abs_dist_from_par, color = "SGD")) +
  geom_line(aes (x = d_mb3$.time, y = d_mb3$abs_dist_from_par, color = "MB")) +
  geom_line(aes (x = d_mom3$.time, y = d_mom3$abs_dist_from_par, color = "Momentum")) +
  geom_line(aes (x = d_ad3$.time,y = d_ad3$abs_dist_from_par, color = "Adam")) +
  my_color_scale+
  my_theme
