...
$$
Our target density is proportional to a log concave function. We choose the $y$'s as $y_1 = -2.5, y_2 = -2, ..., y_8 = 1$ and calculate the $a_i$'s according to CSWR. 
...
```{r, echo = FALSE}
samp_sizes <- c(10, 20, 50, 100, 200, 1000)
plots <- list()

for(i in seq_along(samp_sizes)){
  
  obj <- my_sample(n = samp_sizes[i], sampler = rs_gauss_vec)
  
  c <- alpha_p /obj$Accept
  f_est <- function(y) f_star_vec(y) * c
  
  plots[[i]] <- plot(obj)+
    geom_function(fun = f_est, color = "purple")+
    geom_function(fun = f_true, color = "steelblue")+
    labs(title = paste("Sample Size =", samp_sizes[i]))
}
