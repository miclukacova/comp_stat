...
tibble("Grad in (-20, 200)" = grad_negloglik(-20, 200, 1, data0$x), "Grad in (1,1)" = grad_negloglik(1, 1, 1, data0$x)) %>% 
  kable(caption = "Gradient at different starting points",) %>% 
  kableExtra::kable_styling(full_width = FALSE, position = "center")
```
...
em_bench <- bench::press(
  ...
  {
    bench::mark(
      "GD" =  grad_desc(par = c(2,2), grad = grad_negloglik, H = neg_loglik, x = data$x[1:k], nu = par0$nu),
...
Where $\mathcal{l}(\hat{\theta}) = \sum_{i=1}^N \mathcal{l}_i(\hat{\theta})$.
...
```
This is implemented in the function `fis_inf`. 
...
This empirical Fisher Information paves way for the confidence interval
