...
plot.density_object <- function(obj, h, p = 1, binned = FALSE) {
  dens_data <- as.data.frame(kern_dens(obj, h)[1:2])
  
  if(binned == TRUE){
    dens_data_binned <- as.data.frame(kern_dens_bin(obj, h)[1:2])
    p <- 3
  }
...
test_dens <- function(obj, level = 1e-3){
  h <- seq(0.2,1, length.out = 4)
  for(i in seq_along(h)){
    dens_data <- as.data.frame(kern_dens(obj, h[i])[1:2])
