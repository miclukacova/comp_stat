...
sgd3 <- SGD(par0 = param1$par * 2, grad = grad, gamma = 0.001, x = x1, y = y1, true_par = param1$par, maxit = 300)
```

```{r}
plot(sgd1, 1) + 
  geom_line(aes(x = plot_data(sgd2)$.time, 
                y = plot_data(sgd2)$loss), col = "orange") + 
  geom_line(aes(x = plot_data(sgd3)$.time, 
                y = plot_data(sgd3)$loss), col = "red")+
  ylim(c(264,280))
...
ggplot() +
  ...
  geom_line(aes(x = plot_data(sgd_decay3)$.time, 
                y = plot_data(sgd_decay3)$loss, color = "a = 0.5, n1 = 150, gamma1 = 0.0002")) +
  scale_y_log10() +
  labs(title = "Loss vs Time", x = "Time", y = "Loss")+
  scale_color_manual(values = c("a = 1, K = 0.4" = "red", 
                                "a = 0.7, K = 1" = "orange", 
                                "a = 0.5, n1 = 150, gamma1 = 0.0002" = "black"))+
...
We have also implemented a gradient descent algorithm. We have created an equivalent class for this algorithm as well. We check that the algorithm works:
...
```{r, warning = FALSE, message = FALSE}
gd0 <- GD(par = param$par + rep(0.1,4), H = H, x = x, t0 = 0.05, y = y, true_par = param$par)
sgd0 <- SGD(par0 = param$par + rep(0.1,4), grad = grad, gamma = 0.001, x = x, y = y, true_par = param$par, maxit = 100)
```

```{r}
tibble(" " = c("alpha", "beta", "gamma", "rho"), "True" = param$par, "GD" = gd0$est, "SGD" = sgd0$est)
```
...
bench1 <- bench::press(
  ...
  {
    bench::mark(
      "GD" = grad_desc(par = param$par + rep(0.1,4), grad = grad_gd, H = H, x = x2[1:k], y = y2[1:k]),
...
If we sample from a grid we can exploit the fact that we only have $n$ distinct $x$-values....
