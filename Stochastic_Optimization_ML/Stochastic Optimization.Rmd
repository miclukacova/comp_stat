...
source("~/Desktop/Uni/5aar/CompStat/Assignments/Stochastic Optimization/SO_source.R")
```
...
$$
The $\rho_{n+1}$ is implemented in the function grad. We have implemented two different sampling functions. One of them samples from $\log(x)$'s from a normal distribution and calculates the $y$'s by $y_i = f(x_i|\alpha, \b)

```{r}
N <- 1000
sample0 <- my_sample1(N, par = c(2,1,2,1), omega = 1)
x <- sample0$x
y <- sample0$y


sgd(c(1,1,1,1), grad, N, 0.01, maxiter = 100) 
